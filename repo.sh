...
do
  ...
  dpkg-deb -f "$deb" Package | echo "Depiction: https://repo.samplasion.tk/depictions/?p=$(xargs -0)" >> Packages
  dpkg-deb -f "$deb" Package | echo "SileoDepiction: https://repo.samplasion.tk/repo/pkgs/$(xargs -0).json" >> Packages
  echo "" >> Packages
done
